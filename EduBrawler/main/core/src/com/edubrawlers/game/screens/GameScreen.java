...
public class GameScreen extends AbstractScreen{
    ...
    public void show(){
        ...
        innerTable.row();
        innerTable.add(new Label( playerName1 + "'s TURN!", popupLabelStyle)).left().top().padBottom(50).padLeft(-100).expand();
        innerTable.row();
        ...
        player1Table.row();
        player1Table.add(player1HPLabel).padBottom(120).padLeft(250);
        player1Table.row();
        ...
        player2Table.row();
        player2Table.add(player2HPLabel).padBottom(60).padLeft(250);
        player2Table.row();
            ...
            public void changed(ChangeEvent event, Actor actor) {
                if (isPlayer1Turn) {
                    ...
                    if (currentEquation.getCorrectAnswer() == 0) {
                        ...
                        updatePlayer1GameState(playerName2, playerNameLabel, equationLabel, player2HPLabel, player2HP, damage);
                        ...
                        updatePlayer1GameState(playerName2, playerNameLabel, equationLabel, player2HPLabel, player2HP, 0);
                        setButtonTextsForEquation(currentEquation, optionBtn1, optionBtn2, optionBtn3, optionBtn4);
                    ...
                    if (currentEquation.getCorrectAnswer() == 0) {
                        ...
                        updatePlayer2GameState(playerName1, playerNameLabel, equationLabel, player1HPLabel, player1HP, damage);
                        setButtonTextsForEquation(currentEquation, optionBtn1, optionBtn2, optionBtn3, optionBtn4);
                        ...
                        // Switch to player 1's turn
                        isPlayer1Turn = true;
                        updatePlayer2GameState(playerName1, playerNameLabel, equationLabel, player1HPLabel, player1HP, 0);
                        setButtonTextsForEquation(currentEquation, optionBtn1, optionBtn2, optionBtn3, optionBtn4);
        ...
        pauseBtn.addListener(new InputListener() {
            ...
            public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                if (isPopupVisible) { // If the pause table is not visible
                    ...
                    pauseTable.setFillParent(true);
                    pauseTable.add(pause).width(screenWidth).height(screenHeight).fill();// Add pause table to stage
    ...
    private void updatePlayer1GameState(String playerName2, Label playerNameLabel, Label equationLabel, Label player2HPLabel, float[] player2HP, float damage) {
        currentEquationIndex = (currentEquationIndex + 1) % EquationsSet.EQUATIONS_LIST.size();
        currentEquation = EquationsSet.EQUATIONS_LIST.get(currentEquationIndex);

        equationLabel = (Label) innerTable.getChildren().get(1);
        equationLabel.setText("Equation: " + currentEquation.getEquation());

        // Update the player name label
        playerNameLabel = (Label) innerTable.getChildren().get(0);
    ...
    private void updatePlayer2GameState(String playerName1, Label playerNameLabel, Label equationLabel, Label player1HPLabel, float[] player1HP, float damage) {
        currentEquationIndex = (currentEquationIndex + 1) % EquationsSet.EQUATIONS_LIST.size();
        currentEquation = EquationsSet.EQUATIONS_LIST.get(currentEquationIndex);
